...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train text sentiment model from fusion feature transcripts.")
        ...
        for row in reader:
            ...
            try:
                if label:
                    ...
                    emotion = CANONICAL_EMOTIONS[idx]
                else:
            ...
            rows.append(TextRow(text=text, label_id=label2id[sentiment], language=lang))
            stats["kept"] += 1
            if lang == "zh":
                stats["kept_zh"] += 1
            else:
...
def evaluate(
    ...
    metrics = {
        ...
        "macro_f1": macro_f1_score(labels_np, preds, num_labels=len(SENTIMENT_LABELS)),
        "balanced_accuracy": balanced_accuracy(labels_np, preds, num_labels=len(SENTIMENT_LABELS)),
...
def class_weights_from_rows(rows: list[TextRow]) -> torch.Tensor:
    counts = np.zeros(len(SENTIMENT_LABELS), dtype=np.float64)
    for row in rows:
        counts[row.label_id] += 1.0
    total = float(np.sum(counts))
    weights = np.where(counts > 0.0, total / (len(SENTIMENT_LABELS) * counts), 0.0)
    return torch.tensor(weights, dtype=torch.float32)


def save_confusion(path: Path, y_true: np.ndarray, y_pred: np.ndarray) -> None:
    matrix = build_confusion_matrix(y_true, y_pred, num_labels=len(SENTIMENT_LABELS))
    with path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["truth\\pred", *SENTIMENT_LABELS])
