...
- `uncalibrated ECE`：融合分类头原始概率输出的校准误差。
- `calibrated ECE`：经过温度/分语言校准后的线上候选概率误差。
- Exp03 固定口径：`0.2140`（uncalibrated）对应 `0.0562`（calibrated）。
- Exp01 的 `0.0250` 是该版本 calibrated ECE，不能和 Exp03 的 uncalibrated 直接比较。
